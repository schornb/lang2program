...
def handle_dict_1arg(fn):
    ...
    def wrapped_fn(self, predicate, arg):
        if isinstance(arg, dict):
            answer = {}
            for key, things in arg.iteritems():
...
class TablesPostfixExecutor(Executor):
    ...
    def apply(self, predicate, stack):
        ...
        if self.forbid_partial_empty:
            if (not stack[-1] or (isinstance(stack[-1], dict) and
                all(not x for x in stack[-1].itervalues()))):
    ...
    def apply_join(self, predicate, arg):
        assert isinstance(predicate, basestring), unicode(predicate)
        assert isinstance(arg, (set, InfiniteSet)), unicode(arg)
    ...
    def apply_join_fast(self, predicate, arg):
        if predicate == '!=':
            if isinstance(arg, dict):
                answer = {}
                for key, thing in arg.iteritems():
                    if len(thing) != 1:
    ...
    def apply_end_grow(self, predicate, arg):
        ...
        best_keys = set()
        best_value = None
        for key, values in arg.iteritems():
            for value in values:
