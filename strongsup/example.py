...
class BaseContext(UnicodeMixin):
    ...
    def __unicode__(self):
        return u'\n'.join([unicode(utt) for utt in self.utterances])
...
class Context(BaseContext):
    ...
    def __init__(self, world, raw_utterances):
        ...
        assert isinstance(raw_utterances[0][0], unicode), raw_utterances[0][0]
...
class DelexicalizedContext(BaseContext):
    ...
    def _delexicalize_utterance(self, utt):
        ...
        for predicate, alignment in utt.predicate_alignments.iteritems():
        ...
        for predicate, old_alignment in utt.predicate_alignments.iteritems():
            if not old_alignment:
                ...
                predicate_alignments[predicate] = [
                        ...
                        for (index, strengths) in new_alignment.iteritems()]
